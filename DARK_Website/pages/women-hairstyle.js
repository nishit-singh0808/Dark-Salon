...
export default function ServiceListWomenHairstyle() {
  const [services, setServices] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState("Cut");
  ...
  return (
    <div>
      <h1 className="page-title">Hairstyle Services</h1>
      ...
      >
        {hairstyleSubcategories.map((sub) => (
          <div
            key={sub}
          ...
          >
            <img
              src={`/images/${sub.replace(/\s+/g, "-").toLowerCase()}.jpg`}
              alt={sub}
            />
