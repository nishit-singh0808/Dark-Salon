...
export default function ServiceList() {
  ...
  const [selectedSubcategory, setSelectedSubcategory] = useState("Party Makeup");
    ...
    if (cartItemSnap.exists()) {
      ...
      await setDoc(cartItemRef, {
        ...
        serviceData: {
          name: service.name,
          price: service.price,
          originalPrice: service.original || null,
  ...
  const visibleServices = services.filter(
    (service) =>
      ...
      service.gender === "Women" && // or "Men"
      service.category === "Makeup" // or another category condition
  );

  const getDiscountPercent = (original, price) => {
    if (!original) return 0; // no original price, no discount
    if (price === 0) return 100; // price zero means full discount
    if (!price) return 0; // price undefined or null, no discount
  ...
  return (
    <div>
      <h1 className="page-title">Makeup Services</h1>
      ...
      >
        {makeupSubcategories.map((sub) => (
          <div
            key={sub}
            className={`subservice-item${selectedSubcategory === sub ? " active" : ""}`}
            onClick={() => setSelectedSubcategory(sub)}
          >
            <img
              src={`/images/${sub.replace(/\s+/g, "-").toLowerCase()}.png`}
              alt={sub}
            />
      ...
      <div className="services-list">
        {visibleServices.length === 0 ? (
          <div className="no-services">No services found for "{selectedSubcategory}".</div>
        ) : (
                ...
                <div className="service-card-main">
                  ...
                  <div className="service-card-info">
                    ...
                    <div className="service-card-prices">
                      ...
                      {service.original && (
                        <>
                          <span className="service-card-original-price">₹{service.original}</span>
